...
static void edit_params(int argc, char** argv) {
  ...
  u8 *tmp_dir = getenv("TMPDIR"), *afl_as = getenv("AFL_AS");            // 获取 TMPDIR 和 AFL_AS
  u32 i;
  ...
  if (!tmp_dir) tmp_dir = getenv("TEMP");                               // 获取 TMP 目录路径
  if (!tmp_dir) tmp_dir = getenv("TMP");
  if (!tmp_dir) tmp_dir = "/tmp";

  as_params = ck_alloc((argc + 32) * sizeof(u8*));                      // 为 as_params 分配空间

  as_params[0] = afl_as ? afl_as : (u8*)"as";                           // 设置 as_params[0]

  as_params[argc] = 0;

  for (i = 1; i < argc - 1; i++) {                                      // 遍历 argv[1] 到 argv[argc-1]

    if (!strcmp(argv[i], "--64")) use_64bit = 1;                        // 如果存在 --64 就使得 use_64bit = 1
    ...
    if (!strcmp(argv[i], "-arch") && i + 1 < argc) {
      ...
      if (!strcmp(argv[i + 1], "x86_64")) use_64bit = 1;                 // 如果存在 -arch x86_64 就使得 use_64bit = 1
    ...
    if (clang_mode && (!strcmp(argv[i], "-q") || !strcmp(argv[i], "-Q"))) // 忽略 -q 或者 -Q
      continue;

#endif /* __APPLE__ */

    as_params[as_par_cnt++] = argv[i];                                    // 设置 as_params[as_par_cnt++] = argv[i]
  ...
  if (use_clang_as) {                                                     // 如果 use_clang_as 存在，则设置 -c -x assembler
  ...
  input_file = argv[argc - 1];                                            // 把 argv[argc - 1] 赋给 input_file，即最后一个参数的值为 input_file 的值
  ...
  if (input_file[0] == '-') {
    ...
    if (strncmp(input_file, tmp_dir, strlen(tmp_dir)) &&                  // 将 input_file 与 tmp_dir、/var/tmp/、/tmp/进行比较，如果都不相同，则设置 pass_thru = 1
  ...
  modified_file = alloc_printf("%s/.afl-%u-%u.s", tmp_dir, getpid(),
                               (u32)time(NULL));                          // modified_file = tmp_dir/.afl-getpid()-(u32)time(NULL).s

wrap_things_up:

  as_params[as_par_cnt++] = modified_file;                               // 设置 as_params[as_par_cnt++] = modified_file
  as_params[as_par_cnt]   = NULL;                                        // 结束对 as_params 的编辑
...
static void add_instrumentation(void) {
  ...
  if (ins_lines)
    fputs(use_64bit ? main_payload_64 : main_payload_32, outf);                // 通过 use_64bit判断，向 outf 里写入 main_payload_64 还是 main_payload_32
...
int main(int argc, char** argv) {
  ...
  rand_seed = tv.tv_sec ^ tv.tv_usec ^ getpid();           // 获取随机种子

  srandom(rand_seed);                                      // 设置随机种子

  edit_params(argc, argv);                                 // 编辑相关参数
  ...
  if (getenv(AS_LOOP_ENV_VAR))                             // 获取环境变量 AS_LOOP_ENV_VAR
    FATAL("Endless loop when calling 'as' (remove '.' from your PATH)");

  setenv(AS_LOOP_ENV_VAR, "1", 1);                         // 设置环境变量 AS_LOOP_ENV_VAR 为1

  /* When compiling with ASAN, we don't have a particularly elegant way to skip
     ASAN-specific branches. But we can probabilistically compensate for
     that... */

  if (getenv("AFL_USE_ASAN") || getenv("AFL_USE_MSAN")) {  // 获取环境变量 AFL_USE_ASAN 和 AFL_USE_MSAN，如果有一个存在
    sanitizer = 1;                                         // 则设置 sanitizer = 1
  ...
  if (!(pid = fork())) {
    ...
    execvp(as_params[0], (char**)as_params);               // fork 出一个子进程，执行 execvp(as_params[0], (char**)as_params);
